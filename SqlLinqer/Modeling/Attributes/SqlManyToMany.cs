...
namespace SqlLinqer.Modeling
{
    ...
    /// </summary>
    public sealed class SqlManyToMany : RelationshipAttribute
    {
        ...
        public SqlManyToMany(Type joiningType, string leftJoiningKey, string rightJoiningKey) : base(RelationshipType.ManyToMany)
        {
            directTargets = false;
        ...
        public SqlManyToMany(string joiningTable, string leftForeignerKey, string rightForeignerKey) : base(RelationshipType.ManyToMany)
        {
            directTargets = true;
        ...
        /// <param name="leftForeignerKey">The foreign key on the relational table the is the primary key of the left model</param>
        /// <param name="rightForeignerKey">The foreign key on the relational table the is the primary key of the right model</param>>
        public SqlManyToMany(string joiningTableSchema, string joiningTable, string leftForeignerKey, string rightForeignerKey)
        ...
        public override Relationship ProcessRelationship(Table currentTable, UnprocessedRelationshipMember unprocessed)
        {
            ...
            parent_middle_join.Condition.AddComponent(new WhereTwoColumn(parentTable.PrimaryKey, middle_left_fk, SqlOp.EQ));
            relationship.Joins.Add(parent_middle_join);

            var middle_child_join = new Join(middleTable, childTable);
            middle_child_join.Condition.AddComponent(new WhereTwoColumn(middle_right_fk, childTable.PrimaryKey, SqlOp.EQ));
